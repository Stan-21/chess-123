...
class BitboardElement {
  public:
    // Constructors
    BitboardElement()
        : _data(0) { } // Null constructor for the bitboard
    ...
    void forEachBit(Func func) const {
        if (_data != 0) {
            ...
            while (tempData) {
                int index = bitScanForward(tempData); // Finds the next available bit
                func(index); 
                tempData &= tempData - 1; // Bitwise AND assignment operator (clears the lowest bit)
            }
        }
    }

    BitboardElement& operator|=(const uint64_t other) { // Bitwise OR assignment operator
        _data |= other;
        return *this;
    }
...
private:
    ...
    inline int bitScanForward(uint64_t bb) const {
        ...
        return index;
#else
        return __builtin_ffsll(bb) - 1; // Returns index of first bit with a value (ie 1) (right to left) and then subtracts 1
#endif
    };
